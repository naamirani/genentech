...
class COR101416(Dataset):
    def __init__(
        ...
        self.parent_paths = ["JAKCP200128-COR10-P2", "JAKCP200427-COR14-P2-GFP-HttDye", "COR16"]
        
        #timepoints = {
        #    "JAKCP200427-COR14-P2-GFP-HttDye": ["T1", "T4"],
        #    "COR16": ["T2", "T8"]
        #}
        self. subfolders = {
            "JAKCP200128-COR10-P2": ["20CAG30", "72CAG12"],
            "JAKCP200427-COR14-P2-GFP-HttDye": ["20CAG30", "72CAG12"],
            "COR16": ["20CAG30", "72CAG12"],
        ...
        try:
            ...
            for parent_path in self.parent_paths:
                for subfolder in self.subfolders[parent_path]:
                    print("Globbing files for %s %s, this may take a while..." %(parent_path,subfolder))
                    folder_path = os.path.join(parent_path, "**", subfolder)
                    folder_contents = glob(os.path.join(self.path, folder_path, "*.tif"))
                    print(folder_path)
                    if subfolder in self.control:
                        self.c1.extend(folder_contents)
                    elif subfolder in self.disease:
                        self.c2.extend(folder_contents)
        except:
            #for testing folders
            self.c1 = glob(os.path.join(self.path, "**", "**", "*.tif"))
            self.c2 = glob(os.path.join(self.path, "**", "**", "*.tif"))
